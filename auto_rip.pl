...
# 	
# Version History:
#		  07/21/2014:  updated auto_rip to reflect RegRipper plugin 20140414
#					   - removed userinit, winlivemsn, compatassist, winlivemail, streams
#					   - added startup, cdstaginginfo, reading_locations, winscp, susclient, knowndev, ddo, winevt, processor_architecture
#					   - added malware category
#					   - added the following malware plugins: pending, cpldontload, inprocserver_u, inprocserver, netsvcs 
#					   - moved the appcompatflags plugin from software category to program execution
#					   - added ability to parse AMcache.hve hive (Windows 8) 
#         08/06/2013:  added ability to select the output directory for reports
#         08/06/2013:  removed duplicate sysinternals plugin, replaced * in echo commands with + for Linux compatibility,
#                      and changed report names to include 0 in front of # 
#         05/16/2013:  updated script to account for file name case sensitivity in Linux
#		  04/01/2013:  script created
#
# Script Description:
...
my @all = qw(os users software network storage execution autoruns log malware web user_config user_act user_network user_file user_virtual comm); 
...
Copyright 2014  Corey Harrell (jIIr)
EOT
exit;
}
...
sub execution	{
	...
	# build filename for report 
	my $executionreport = $reportdir . "06_program_execution_information.txt"; #filename for the execution report
...
sub autoruns	{
	...
	# build filename for report 
	my $autorunsreport = $reportdir . "07_autoruns_information.txt"; #filename for the autoruns report
...
sub log	{
	...
	# build filename for report 
	my $logreport = $reportdir . "08_log_information.txt"; #filename for the log report
...
sub malware	{
	...
	# build filename for report 
	my $malwarereport = $reportdir . "09_malware_indicators.txt"; #filename for the malware report
	...
	if ($usrclassdir) { 
		...
		my $usrclass = $usrclassdir . $usrclass_var;
		#running select plugins
		system (qq{perl rip.pl -r "$usrclass" -p inprocserver_u >>"$malwarereport" 2>>"$logfile"});
		system (qq{echo ...........................................................................................................>>"$malwarereport"});
		system (qq{echo .>>"$malwarereport"});
	}
}
sub web	{
	...
	# build filename for report 
	my $webreport = $reportdir . "10_web-browsing_information.txt"; #filename for the web report
...
sub user_config	{
	...
	# build filename for report 
	my $userconfigreport = $reportdir . "11_user-configuration_information.txt"; #filename for the user_config report
...
sub user_act	{
	...
	# build filename for report 
	my $useractreport = $reportdir . "12_user-account-general-activity.txt"; #filename for the user_act report
...
sub user_network	{
	...
	# build filename for report 
	my $usernetworkreport = $reportdir . "13_user-account-network-activity.txt"; #filename for the user_network report
...
sub user_file	{
	...
	# build filename for report 
	my $userfilereport = $reportdir . "14_user-account-file-access-activity.txt"; #filename for the user_file report
...
sub user_virtual	{
	...
	# build filename for report 
	my $uservirtualreport = $reportdir . "15_user-account-virtual-access.txt"; #filename for the user_virtual report
...
sub comm	{
	...
	# build filename for report 
	my $commreport = $reportdir . "16_communications_information.txt"; #filename for the comm report
